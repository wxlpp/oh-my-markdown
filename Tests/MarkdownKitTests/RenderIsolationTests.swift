...
struct RenderIsolationTests {
    ...
    @Test @MainActor func materializationRetainsResourceOwnerAndPointGeometry() throws {
        ...
        weak var observedOwner: RenderedResourceLease?
        var snapshot: RenderSnapshot?
        do {
            let owner = RenderedResourceRecord(image: image, baselineOffset: -2).acquireLease()
            observedOwner = owner
    ...
    @Test @MainActor func staticResourcesKeepIdentityAndPreparationChecksConfiguration() throws {
        ...
        #expect(try model == (RenderPreparer(configuration: configuration).prepare(input)))
...
extension RenderIsolationTests {
    @Test @MainActor func encodedResourceCrossesActorsPreservingPixelsAlphaAndPointGeometry() async throws {
        let encoded = try await Task.detached {
            ...
            return try RenderedImage(cgImage: #require(context.makeImage()), pointSize: CGSize(width: 2, height: 1))
        }.value
        let transported = await Task.detached { encoded }.value
        #expect(transported.encodedData == encoded.encodedData)
        #expect(transported.pointSize == CGSize(width: 2, height: 1))
        let image = try transported.materialize()
        ...
        #endif
        #expect(pixels.width == 4 && pixels.height == 2)
        let verification = try #require(CGContext(
            data: nil,
            width: 4,
            height: 2,
            bitsPerComponent: 8,
        ...
        verification.draw(pixels, in: CGRect(x: 0, y: 0, width: 4, height: 2))
        let bytes = try #require(verification.data).assumingMemoryBound(to: UInt8.self)
        #expect(abs(Int(bytes[0]) - 128) <= 1)
        #expect(bytes[1] == 0)
        #expect(bytes[2] == 0)
        #expect(abs(Int(bytes[3]) - 128) <= 1)
        #expect(throws: RenderedImage.Failure.invalidDimensions) {
            try RenderedImage(encodedData: encoded.encodedData, pointSize: CGSize(width: CGFloat.nan, height: 1)).materialize()
        }
        #expect(throws: RenderedImage.Failure.invalidEncodedData) {
            try RenderedImage(encodedData: Data([0]), pointSize: CGSize(width: 1, height: 1)).materialize()
        }
    }
}
