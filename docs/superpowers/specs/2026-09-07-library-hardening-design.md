...
Decoded image residency uses an explicit `@MainActor` accounting ledger and LRU, not `NSCache.totalCostLimit`. The hard process-wide resident decoded budget is 192 MiB across decode outputs awaiting publication and completed cached images; the completed-cache sublimit is 128 MiB. Before decode, the coordinator reserves the estimated downsampled output cost and synchronously evicts LRU completed entries as needed; if sufficient hard budget is still unavailable, decode waits without holding a decode slot. On successful insertion, the in-flight cost is atomically reclassified as completed-cache cost without temporarily double-counting it. Insertion evicts or rejects before either hard limit can be exceeded. Memory pressure clears completed entries. The ledger/cache is injectable for deterministic tests.
...
- Image transfer/decode concurrency, upfront encoded reservations, hard decoded-residency accounting, completed caches, and all negative caches enforce the §7 budgets under 100 distinct valid requests; two 17 MiB and four 9 MiB concurrent bodies cannot hold-and-wait.
