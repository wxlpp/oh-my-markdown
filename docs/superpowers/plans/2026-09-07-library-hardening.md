...
- `MarkdownCopyGranularity.exact` means the copy covers the selection and no more; it does **not** promise byte-identical original source for every block. `InlineNode` carries no source ranges — only blocks do — so a partial selection inside a block cannot be extracted and is reported `.blockExpanded` instead. Giving inline runs real source offsets is a `MarkdownCore` change and was out of this task's file list.
- Blocks the parser rebuilt (a block formula lifted out of a paragraph, a paragraph containing inline math) carry `sourceRange == nil`. Their bytes are recovered by bracketing: consecutive blocks are contiguous in the source, so a run of source-less blocks spans exactly the gap between the ranges around it, trimmed of separators. This replaces the previous all-or-nothing fallback, which lost the whole selection when a *boundary* block lacked a range. It assumes block order matches source order; if a future transform reorders blocks, the bracketing silently captures the wrong span.
- Reconstructed syntax (`$$latex$$`, `![alt](source)`, ```` ```svg ````) is used only when there is no original source to bracket at all — programmatic `MarkdownDocument(parsedBlocks:)` input. It is not byte-identical to what an author wrote (spacing inside delimiters is lost), and that case reports `.renderedFallback`.
- The first `xcodebuild test` of the Example scheme after `Package.swift` gained `resources:` failed every UI test with `Cannot launch simulated executable: no file found at …/Example.app`, although the same log shows the app linked, signed and copied. Two subsequent runs passed unchanged. Not reproduced, so the cause is unconfirmed — most likely a stale incremental install from the newly appearing `MarkdownKit_MarkdownPlatformView.bundle`. Logs: `.artifacts/task-9-example.log` (failing) and `.artifacts/task-9-example-retry.log`. If it recurs in CI, clean DerivedData for the Example project before the run.
- `MarkdownCopyCommandTitle.markdownSource` is a mutable static, which is how a host overrides the command title. It is `@MainActor`, so it is not a data race, but it is process-global: two hosts in one process cannot differ.
