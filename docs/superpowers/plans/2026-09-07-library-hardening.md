...
- macOS exposes headings with an `AXHeading` role and speaks the level through the value: `NSAccessibilityElement` has no level setter. iOS uses `.header` traits.
...
- macOS scroll observers are registered per overlay and removed with it. They were previously never removed, so registrations accumulated one per overlay recreation — every width and style change — for the life of the view. iOS is clean by construction, since `UIScrollView.delegate` is weak.
