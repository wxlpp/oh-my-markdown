...
- The **end** boundary of a block the parser rebuilt is not provable: such a block records no end, and the bytes after it may belong to nothing. It is therefore used only when the block owns a real range, or when the selection runs to the end of the document. Otherwise the selected blocks are reconstructed and the result is `.renderedFallback`. The cost is real: selecting a mid-document formula returns reconstructed `$$latex$$` rather than the author's bytes, so spacing inside the delimiters is lost. Recovering it needs a recorded end for rebuilt blocks — a `MarkdownCore` change, the same one that would give inline runs their offsets.
- Reconstruction (`$$latex$$`, `![alt](source)`, ```` ```svg ````) is approximate — emphasis and links have already lost their delimiters — and is **always** reported `.renderedFallback`; no path calls it source. It joins blocks with a blank line, because a single newline is one paragraph in Markdown and the copy has to round-trip as the blocks it came from.
...
- `markdownSourceForRenderedSelection` is the pre-Task-9 mapping and no shipping command calls it; it is kept only because `RenderMigrationParityTests` pins the legacy behaviour deliberately. The per-view `_copiedStringForCurrentSelection*` seams are deleted, and the assertions that rested on them now drive `markdownSourceSelectionResult()`.
