...
- `MarkdownCopyGranularity.exact` means the copy covers the selection and no more; it does **not** promise byte-identical original source for every block. `InlineNode` carries no source ranges — only blocks do — so a partial selection inside a block cannot be extracted and is reported `.blockExpanded`. Giving inline runs real source offsets is a `MarkdownCore` change and was out of this task's file list.
- Blocks the parser rebuilt (a block formula lifted out of a paragraph, a paragraph containing inline math) carry `sourceRange == nil`. `MathBackfill.resolve` nils it for **every** block it emits at a rewritten position, so a document whose paragraphs each contain inline math is entirely source-less — this is the common case, not a corner. Their bytes are recovered by bracketing the gap between the ranges around them, and the result is a single verbatim span from one boundary byte to the other, so interior bytes (indentation, separator lines, list markers) are preserved exactly. Two conditions make this sound and both are enforced: the source-less run must *start* where the selection starts and *end* where it ends (a run reaching further would hand over bytes of blocks the reader never selected — review round 1 measured this returning a whole document as `.exact`), and only an edge a bracket produced is trimmed (a block's own range keeps its leading whitespace, because four spaces are the difference between a code block and a paragraph — review round 1 measured an indented code block being corrupted and reported `.exact`). Both are pinned by tests named for the defect. The remaining assumption is that block order matches source order; a future transform that reorders blocks would silently bracket the wrong span.
- Reconstructed syntax (`$$latex$$`, `![alt](source)`, ```` ```svg ````) is used only when no byte span reaches the selection's edges — programmatic `MarkdownDocument(parsedBlocks:)` input, or a source-less run continuing past the selection. It is approximate (spacing inside delimiters is lost, emphasis and links have already lost their delimiters) and is **always** reported `.renderedFallback`; there is no path on which reconstruction is called source.
- `.markdownCopyText` substitution is only valid for one-character runs, because `renderedCopyText` emits the whole value for any sub-range that touches it. Decorative text that a reader *does* see — the `🖼 ` marker on an unloaded image — is dropped with `.markdownCopySkip` instead, so a copy does not change meaning depending on whether the image had loaded, and partial selections still slice normally. Pinned by `everyCopyTextRunIsExactlyOneCharacter`, which caught a violation the first time it ran.
- On iOS the localized source-copy command reaches the **main menu only** (the iPad and Mac Catalyst menu bar). The iPhone selection callout is presented by `UITextInteraction`, not built from this responder's `buildMenu(with:)`, so the command does not appear there; SwiftUI hosts reach it through `MarkdownSelectionProxy`. Adding a `UIEditMenuInteraction` alongside the one `UITextInteraction` manages was not attempted because a double-presented callout could not be verified headlessly in this environment. macOS has a real context menu with both commands.
- `MarkdownCopyCommandTitle` is a mutable `@MainActor` static, which is how a host overrides the titles. Safe from data races, but process-global: two hosts in one process cannot differ. An environment entry alongside `markdownSelectionProxy` would match the house pattern.
- `Package.swift` gained `resources:` and `defaultLocalization`, so every consumer now gets a `MarkdownKit_MarkdownPlatformView` resource bundle. That is an integration-surface change and belongs in the 0.1.x → 0.2.0 migration notes.
- The parity goldens strip the three copy keys before comparing (`strippingCopyMetadata`), because they change no glyph but do split runs at boundaries the fixtures never had. The cost is that the goldens no longer pin copy-run structure; `everyCopyTextRunIsExactlyOneCharacter` is what covers the invariant that mattered.
- The first `xcodebuild test` of the Example scheme after `Package.swift` gained `resources:` failed every UI test with `Cannot launch simulated executable: no file found at …/Example.app`; two later runs passed unchanged. **The cause is unknown, and the log rules out the stale-install explanation first recorded here**: `ExampleTests/smokeMarkdownParsesOneHeading()` — hosted *in* `Example.app` — passed on Clone 1 in that same run, so the app was present and launchable. Only Clone 2, the UI-test runner, failed, and immediately before the first failure the log shows `IDELaunchParametersSnapshot: … DebuggerLLDB.DebuggerVersionStore.StoreError error 0` and `no debugger version`, suggesting launch-parameter resolution failed first and "no file found" is the downstream symptom. Logs: `.artifacts/task-9-example.log` (failing) and `.artifacts/task-9-example-retry.log`.
