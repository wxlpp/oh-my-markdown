...
- Deviation from this task's Step 3: the plan specifies `RenderSessionEvent.replaceLinkConfiguration(policyID:handlerID:)` and "policy or handler replacement bumps the session generation". The implemented case carries no payload and the configuration generation is not bumped. Reason: the driver is the sole owner of the live policy/handler, so shipping the IDs into the session would create a second copy that can disagree with it, and `configurationGeneration` is also bumped by width and style changes, which made a generation-based activation guard reject decisions no replacement had invalidated. Activation instead revalidates `linkConfigurationRevision`, a counter bumped by every install of a link configuration and by nothing else.
- Installing a link configuration whose policy and handler identities are unchanged still routes through `send(.replaceLinkConfiguration)` only when an identity differs, but the mutation the session receives causes a full document reparse for state the session does not read. Deliberate: the session's mutation channel has no cheaper "driver-only" lane today, identity-equal installs are already filtered out before the send, and adding a lane touches Task 4's mutation contract. If Task 12 measures reparse cost on link-configuration churn, this is the first candidate.
...
- `Scripts/check-link-activation.sh` inventories openers by name within `Sources`, so a call assembled from string interpolation or reached through `NSClassFromString` is not caught, and `Tests` is out of scope by design. Both limits are stated in the script.
