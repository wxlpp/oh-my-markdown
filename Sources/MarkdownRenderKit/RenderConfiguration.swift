...
public struct MarkdownRenderConfiguration {
  ...
  public static var `default`: Self { Self(defaultStyle: .default) }
  ...
  public func snapshot(generation: UInt64) -> RenderConfigurationSnapshot {
    if let resolvedDefault {
      return RenderConfigurationSnapshot(
        id: configurationID, typography: resolvedDefault.typography, colors: resolvedDefault.colors,
        spacing: resolvedDefault.spacing, generation: generation,
        mathScale: resolvedDefault.mathScale)
    }
    return style.snapshot(
      generation: generation, configurationID: configurationID,
      usesPreferredMetrics: usesPreferredMetrics)
  }
}
