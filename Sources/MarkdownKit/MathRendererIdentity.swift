...
/// 不对外暴露 API。逻辑与 round-1 的 `x.configurationID == y.configurationID` 身份语义逐字节一致。
/// (Copilot PR #4 R5 #1/#2)
///
...
func isSameMathRenderer(_ a: MathRendererConfiguration?, _ b: MathRendererConfiguration?) -> Bool {
    switch (a, b) {
    case (nil, nil): true
